...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// Gera uma senha temporária forte e aleatória para contas cuja senha salva
// era fraca demais para o Firebase Authentication aceitar (mínimo 6
// caracteres). Só usada nesse caso específico, uma única vez.
// ---------------------------------------------------------------------------
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const temporaryPasswordsIssued: { id: string; email: string; newPassword: string }[] = [];
    ...
    for (const docSnap of snapshot.docs) {
      ...
      try {
        ...
        if (err.code === 'auth/uid-already-exists') {
          ...
          // Senha salva era fraca demais (menos de 6 caracteres). Em vez de
          // deixar essa conta insegura no ar, geramos uma senha temporária
          // forte agora, criamos a conta real com ela, e reportamos essa
          // senha uma única vez na resposta — repasse-a com segurança para
          // a pessoa (por exemplo, por WhatsApp) e peça para trocar assim
          // que possível.
          ...
          try {
            ...
            migrated.push(userId);
            temporaryPasswordsIssued.push({ id: userId, email: data.email, newPassword: tempPassword });
          } catch (err2: any) {
            conflicts.push({ id: userId, reason: `Falha ao gerar senha temporária: ${err2.message || 'erro desconhecido'}.` });
            continue;
          }
        } else {
    ...
    return res.status(200).json({
      ...
      summary: {
        ...
        conflitos: conflicts,
        senhasTemporariasGeradas: temporaryPasswordsIssued,
